...
LOOPTY			BCC EACHFRAME		; next frame in sequence

				BCS STARTANIMATION		;hit EOF, restart
...
BLOAD
				...
				STA namlo
				LDA #3
				STA sizehi			;pre-read needed
				LDA #$C0
				STA sizelo			;load first frame
				LDA #>BEGINDATA
				STA ldrhi
				LDA #<BEGINDATA
				STA ldrlo
				JMP hddopendir			;open "data"


FILENAME		DB	ENDNAME-NAME 			;Length of name
NAME    		ASC	'DATA' 				;followed by the name
ENDNAME 		EQU	*
